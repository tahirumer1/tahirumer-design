...
// Nexen Labz — the live studio site Tahir designed & built. Embedded as the
// FIRST Development project (its detail page iframes the live site). Defined
// here (not in the auto-generated demoProjects.js) so it survives regeneration.
const NEXEN_LABZ = {
  _id: "demo-nexenlabz",
  slug: "nexenlabz",
  title: "Nexen Labz — Digital Studio",
  category: "Development",
  year: "2026",
  featured: false,
  order: 99,
  accentColor: "#8B5CF6",
  brief: "Conversion-focused websites that earn their keep.",
  role: "UI/UX Design · Web Design · Development · Branding",
  roleTags: ["UI/UX Design", "Web Design", "Development", "Branding"],
  platform: "Next.js · Vercel",
  niche: "Digital Studio",
  description:
    "A full-service digital studio with teams in Calgary and Lahore — design, web development, and growth under one team. Conversion-focused websites and brands for clients across North America, the UK, and the Gulf.",
  challenge: "",
  context: "",
  outcome: "",
  process: [],
  thumbnailUrl: "/demos/nexenlabz/card.jpg",
  detailImageUrls: [],
  demoUrl: "https://www.nexenlabz.com/",
};
// perch — a self-directed storefront concept, built end-to-end as a five-page
// flow and self-hosted at /demos/perch/. Defined here (not in the
// auto-generated demoProjects.js) so it survives regeneration.
const PERCH = {
  _id: "demo-perch",
  slug: "perch",
  title: "perch — Storefront Concept",
  category: "Development",
  year: "2026",
  featured: false,
  order: 98,
  accentColor: "#00786b",
  brief: "Product discovery that floats instead of listing.",
  role: "UI/UX Design · Design System · Front-end",
  roleTags: ["UI/UX Design", "Design System", "E-commerce", "Front-end"],
  platform: "Vite · Vanilla JS",
  niche: "E-commerce Concept",
  description:
    "A self-directed storefront concept exploring product discovery as a floating constellation rather than a grid — then carried through the whole flow: category listing with live filters and sort, product detail with variants, bag, and checkout. Built as five pages on one shared shell, with bag state in localStorage so the flow genuinely works. Fictional brand; the checkout is a visual demonstration that takes no payment.",
  challenge: "",
  context: "",
  outcome: "",
  process: [],
  thumbnailUrl: "/demos/perch/card.jpg",
  detailImageUrls: [],
  demoUrl: "/demos/perch/index.html",
};

// Hand-authored UI/UX case studies first, then the two hand-authored builds,
// then the 16 bundled demos.
const DEMO_ALL = [...CASE_STUDIES, NEXEN_LABZ, PERCH, ...DEMO_PROJECTS];
...
async function resolveMedia(ids) {
  ...
  // The WP REST API caps `per_page` at 100, and `include` honors that cap — so a
  // single request with >100 ids silently returns only the 100 NEWEST media and
  // drops the rest. With sliced detail images the whole-archive query asks for
  // ~230 ids, which used to truncate older thumbnails into null. Fetch in chunks.
  const map = new Map();
  for (let i = 0; i < uniq.length; i += 100) {
    const chunk = uniq.slice(i, i + 100);
    const media = await wpJson(`/wp/v2/media?include=${chunk.join(",")}&per_page=100&_fields=id,source_url,media_details`);
...
export async function getAllProjects() {
  // The 16 static live-demo projects (category "Development") are appended after
  // the WordPress case studies (their order is 100+, so they always sort last).
  if (isWpConfigured()) {
    try {
      const projects = await wpAllProjects();
      if (projects.length) return [...projects, ...DEMO_ALL];
    } catch (e) {
      console.warn("WordPress projects failed, using fallback:", e.message);
    }
    return [...FALLBACK_PROJECTS, ...DEMO_ALL];
  }
  const base = await fetchWithFallback(allProjectsQuery, FALLBACK_PROJECTS);
  return [...base, ...DEMO_ALL];
}
...
export async function getProjectBySlug(slug) {
  const demo = DEMO_ALL.find((p) => p.slug === slug);
  if (demo) return demo;
  if (isWpConfigured()) {
    try {
...
export async function getProjectSlugs() {
  const demoSlugs = DEMO_ALL.map((p) => p.slug);
  if (isWpConfigured()) {
    try {
      const posts = await wpJson(`/wp/v2/project?per_page=100&_fields=slug`);
      if (posts.length) return [...posts.map((p) => p.slug), ...demoSlugs];
    } catch (e) {
      console.warn("WordPress slugs failed, using fallback:", e.message);
    }
    return [...FALLBACK_PROJECTS.map((p) => p.slug), ...demoSlugs];
  }
  if (!isSanityConfigured()) return [...FALLBACK_PROJECTS.map((p) => p.slug), ...demoSlugs];
  try {
    const slugs = await sanityClient.fetch(projectSlugsQuery);
    return [...(slugs?.length ? slugs : FALLBACK_PROJECTS.map((p) => p.slug)), ...demoSlugs];
  } catch {
    return [...FALLBACK_PROJECTS.map((p) => p.slug), ...demoSlugs];
  }
}

// Genuine client reviews, defined in code (like NEXEN_LABZ above) so they always
// lead the carousel and can carry a verified rating + source — fields the ACF-free
// WordPress testimonial CPT can't store. These render with 5★ + a "via <source>"
// badge; the WP placeholder testimonials have neither, so a real review stands out.
const REAL_TESTIMONIALS = [
  {
    _id: "real-upwork-webpress",
    quote:
      "I enjoyed working with Tahir, he is a sharp young man with a bright future. Our communication was great. He is committed to quality work and shared goals. It was a successful project and I look forward to hiring him in the future. It was one of my best outlook experiences after many years of hiring talent here.",
    author: "Verified Upwork Client",
    company: "Author Website Development",
    rating: 5,
    source: "Upwork",
    _order: 0,
  },
...
export async function getTestimonials() {
  let base;
  if (isWpConfigured()) {
    try { base = await wpTestimonials(); }
    catch (e) { console.warn("WordPress testimonials failed, using fallback:", e.message); base = FALLBACK_TESTIMONIALS; }
  } else {
    base = await fetchWithFallback(testimonialsQuery, FALLBACK_TESTIMONIALS);
  }
  // Real reviews first, then the existing (placeholder) ones.
  return [...REAL_TESTIMONIALS, ...base];
}
